...
# 划分数据集为训练集和测试集
sentences_train, sentences_test, y_train, y_test = train_test_split(sentences, labels_number, test_size=0.3, random_state=42)
print(Counter(labels_number))
# 把句子转化为特征向量
vectorizer = TfidfVectorizer()
X_train = vectorizer.fit_transform(sentences_train).toarray()
X_test = vectorizer.transform(sentences_test).toarray()
# print(len(sentences_test),len(X_test))
...
print("\tRecall: %1.3f" % recall_score(y_test, y_pred,zero_division=1))
